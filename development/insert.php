...
<?php
 ...
 if ($mode=="modify"){
     ...
     try{
        $sql = "select * from freeboard_db.development where num=?";  // get target record
      ...
      // unlink 함수: 인자에 있는 파일을 삭제한다(서버에 저장되어 있는 물리적 위치에 존재하는 파일을 삭제한다)
		unlink($delete_path);
		   ...
		   $pdo->beginTransaction(); 
                   $sql = "update freeboard_db.development set $field_org_name = ?, $field_real_name = ?  where num=?";
                   $stmh = $pdo->prepare($sql); 
     ...
     try{
        $pdo->beginTransaction();   
        $sql = "update freeboard_db.development set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql); 
 ...
 try{
     $pdo->beginTransaction();
     $sql = "insert into freeboard_db.development(id, name, nick, subject, content, regist_day, hit, is_html, ";
