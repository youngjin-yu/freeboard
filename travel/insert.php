...
<?php
 ...
 if(isset($_REQUEST["html_ok"]))  //checkbox는 체크해야 변수명 전달됨.
  ...
  $upload_dir = $root.'/freeboard/travel/data/';   //물리적 저장위치    
 
 //수정인 경우   
 if ($mode=="modify"){
   
   try{
      $sql = "select * from freeboard_db.traveldata where num = ? ";
     ...
     //  $copied_name[2] = $row["file_copied_2"];
      ...
      try{
       ...
       $sql = "update freeboard_db.travel set subject = ?, content =?, is_html =? where num = ?";  
       ...
       try{
         $pdo->beginTransaction();
         $sql = "delete from freeboard_db.traveldata where num = ?";  
     ...
     for ($i=0; $i<$count; $i++)
     {
      ...
      if(move_uploaded_file($upfile_tmp_name[$i], $uploaded_file[$i])) { //$_FILES["upfile"]["tmp_name"][$i]
         $sql = "INSERT INTO freeboard_db.traveldata(num,nick,file_name,file_copied)
   ...
   // modify가 아닌 경우 : insert인 경우를 의미(신규 글쓰기인 경우)
      //2020.05.19+(s)
  try{
   $sql = "select * from freeboard_db.travel order by num desc limit 1";
   $stmh = $pdo->prepare($sql); 
   ...
   if($rowcount==0 || $row["num"]==$num){  
      if ($html_ok =="y"){
        ...
        try{
          ...
          $sql = "insert into freeboard_db.travel(id, name, nick, subject, content, regist_day, hit, is_html)";
  ...
  //2020.05.19+(e)
   ...
   header("Location:http://localhost/freeboard/travel/list.php");
 ?>